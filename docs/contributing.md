...
زبان تیم فارسی است؛ شناسه‌ها، env، مسیر HTTP و نام نوع انگلیسی می‌مانند (ترجیحاً داخل backticks).

- بالای فایل غیربدیهی: بلوک کوتاه `/** … */` — این فایل چیست، نفر بعدی چه کار نباید بکند، چرا تصمیم غیرواضح گرفته شده. بدون جعبهٔ ASCII، بدون فهرست Public API، بدون «Responsible for»، بدون خط‌چین تزئینی (`// ───`).
- روی export عمومی: یک خط `/** هدف + محدودیت یا نگاشت Nest */` اگر از اسم مشخص نیست.
- داخل بدنه: `//` یک خط، فقط «چرا» وقتی کد خلاف انتظار است (quirk لایو Nest، تلهٔ ky، race توکن، …).
- ایموجی در کامنت نگذارید. کامنت انگلیسیِ بازگویی کد را فارسیِ کوتاه کنید یا اگر بدیهی است حذف کنید.
