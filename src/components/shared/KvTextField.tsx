  ...
  ].join(' '),
  {
    variants: {
      ...
      state: {
        ...
        error: 'border-rose-300 focus-within:border-rose-400 focus-within:ring-rose-500/15',
        locked:
...
export type KvTextFieldProps = {
  ...
  error?: string;
  /** Helper / hint text (not error) */
  hint?: string;
  /**
   * Locked = visually obvious disabled/read-only:
   * slate-50 bg, muted text, not-allowed cursor, optional Lock icon
   */
  locked?: boolean;
 ...
 * Placeholder color is always slate-400 (#94a3b8) per `.kv-input::placeholder`.
 *
 * Prefer RHF `register` (uncontrolled) for form text inputs. Use controlled
 * values only for display-only locked fields or non-native controls elsewhere.
 */
export const KvTextField = React.forwardRef<HTMLInputElement, KvTextFieldProps>(
  function KvTextField(
    ...
    const state: KvTextFieldState = locked ? 'locked' : error ? 'error' : 'default';
    ...
    return (
      <div className="w-full font-sans" data-slot="kv-text-field">
        ...
        >
          ...
          {locked && showLockIcon ? (
            <div className="flex shrink-0 items-center pe-3 text-slate-400">
