...
const MESSAGE_STYLES: Record<AuthFormMessageState['type'], { container: string; icon: string; Icon: typeof CircleAlert }> = {
...
/** Dismissible feedback banner shown above the auth forms (mirrors `auth_formMessage` in the mockup). */
export function AuthFormMessage({ message, onDismiss }: AuthFormMessageProps) {
  ...
  return (
    <div className={cn('mb-4 flex w-full items-start gap-3 rounded-xl border p-3.5 text-start transition-all', container)}>
